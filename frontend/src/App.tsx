...
function App() {
  ...
  const initialScenario: ScenarioData = {
    ...
    vehicle_skills: Object.fromEntries(
      Object.entries(exampleScenario.vehicle_skills).map(([k, v]) => [
        String(k),
        v as string[],
      ]),
    ),
  ...
  const [numVehicles, setNumVehicles] = useState<number>(
    initialScenario.num_vehicles,
  );
  const [availableSkills, setAvailableSkills] = useState<string[]>(
    initialScenario.available_skills,
  );
  const [vehicleSkills, setVehicleSkills] = useState<{
    [key: string]: string[];
  }>(initialScenario.vehicle_skills);
  const [selectedNode, setSelectedNode] = useState<VRPNode | null>(
    initialScenario.nodes[0] || null,
  );
  ...
  const handleSkillsChange = useCallback(
    (
      ...
      newVehicleSkills: { [key: string]: string[] },
    ) => {
      ...
      setNodes((prevNodes) =>
        prevNodes.map((node) => ({
          ...node,
          required_skills: node.required_skills?.filter((skill) =>
            newAvailableSkills.includes(skill),
          ),
        })),
      );
